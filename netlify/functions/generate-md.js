...
## 🎨 결과생성기용 특수 태그 (필수!)
보고서에 아래 태그를 적절히 사용하세요. 결과생성기(6_result.html)에서 예쁜 박스로 변환됩니다.
...
### 태그 사용 규칙
1. 각 섹션마다 최소 1-2개 태그 사용
2. <!-- 소름 --> 태그는 정말 정확한 부분에만 (남용 금지)
3. <!-- 왜그런지 --> 태그는 사주 근거 설명에 필수
4. <!-- 조언 --> 태그는 실용적 해결책에 사용
5. <!-- 마무리멘트 --> 태그는 보고서 마지막에 1번만

### 기타 형식
- **키워드 태그**: \`[키워드] 재물운, 변화, 도전\` → 예쁜 태그로 변환
- **테이블**: 마크다운 테이블 형식 사용 가능
- **굵게/이탤릭**: \`**굵게**\`, \`*이탤릭*\` 사용 가능
`;
